...
import com.DCHZ.TYLINCN.entity.PDetailFBFInfoEntity;
...
import com.DCHZ.TYLINCN.entity.PDetailYSHTInfoEntity;
...
public class FenBaoFangDetailActivity extends BaseNormalActivity{
	...
	private final int FLAG_HEGE=0x104;
	...
	private RspFenBaoFangDetailEntity rsp;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initExtras();
		initLayout();
		registMsgRecevier(EventCommon.EVENT_FENBAOFANG);
		...
		registMsgRecevier(EventCommon.EVENT_IS_HEGE);
		int seq=ProtocalManager.getInstance().getFenBaoFangTDetail(entity);
		mReqList.add(seq);
		showLoading();
	}
...
//		mMsgPage.setRefreshListener(mRefreshListener);
		if(this.type==Common.TYPE_DAIBAN){
			...
			mBottomView.setClickListener(new IDaiBanClickListener() {
				...
				public void typeClickListener() {
					...
					IntentUtils.starTiJiaoActivity(FenBaoFangDetailActivity.this, GET_TYPE,noTag);
				}
				...
				public void nextClickListener() {
					...
					IntentUtils.starJieShouRenActivity(FenBaoFangDetailActivity.this, mList, GET_JIESHOUREN);
				}
				
				@Override
				public void btnClickListener() {
					// TODO Auto-generated method stub
					String str=mHeader.getValue();
					try {
					...
					int seq1=ProtocalManager.getInstance().getIsHeGe(entity.YWID,str);
					mReqList.add(seq1);
	...
	public void handleReceiveMsg(int eventId, int seqNo, Object obj) {
		// TODO Auto-generated method stub
		if(eventId==EventCommon.EVENT_FENBAOFANG){
			if(obj instanceof RspFenBaoFangDetailEntity){
				RspFenBaoFangDetailEntity rsp=(RspFenBaoFangDetailEntity) obj;
		 ...
		 if(eventId==EventCommon.EVENT_IS_HEGE){
			 if(obj instanceof RspIsHeGeEntity){
				 RspIsHeGeEntity rsp=(RspIsHeGeEntity) obj;
				 Message msg=Message.obtain();
					msg.obj=rsp;
					msg.what=FLAG_HEGE;
					sendMsg(msg);
	...
	protected void handleMsg(Message msg) {
		...
		switch (what) {
		case FLAG_SET_TOP:
			 rsp=(RspFenBaoFangDetailEntity)msg. obj;
			if(rsp!=null&&rsp.isSucc){
//				List<PShenPiInfoEntity> mList=rsp.mEntity.BXMXInfo;
				ArrayList<PDetailFBFInfoEntity> bxInfo=rsp.mEntity.HeGeGongFangInfo;
				if(bxInfo!=null&&bxInfo.size()>0)
				mHeader.setItem(bxInfo.get(0).GFDengJiBuMen);
				View view=getHeaderView(bxInfo);
				mMsgPage.addHeaderView(view);
				int seq1=ProtocalManager.getInstance().getBanLiYiJian(entity.SLID);
				mReqList.add(seq1);
				boolean readOnly=bxInfo.get(0).ShuXing.get(0).ReadOnly;
				MyLog.debug(TAG, "[handleMsg]  readOnly:"+readOnly);
				if(readOnly||this.type==Common.TYPE_YIBAN){
					mHeader.setValue4(bxInfo.get(0).GFPingShenJieLun);
				}else{
					mHeader.showCheckBox();
					if("合格".equals(bxInfo.get(0).GFPingShenJieLun)){
						isHege="合格";
						mHeader.setValue4(isHege);
						mHeader.setCheckBox(true);
					}else{
						isHege="不合格";
						mHeader.setValue4(isHege);
						mHeader.setCheckBox(false);
					}
				}
			}
			break;
		...
		case FLAG_HEGE:
			hideLoadingDialog();
			RspIsHeGeEntity rsp3=(RspIsHeGeEntity) msg.obj;
			if(rsp3!=null&&rsp3.isSucc){
...
//			showToast(mBLUserName);
	...
	private View getHeaderView(ArrayList<PDetailFBFInfoEntity> bxInfo){
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			for(int i=0;i<bxInfo.size();i++){
				...
				mList.add(bxInfo.get(i).GFMingCheng);
				mList.add(bxInfo.get(i).GFDiZhi);
				mList.add(bxInfo.get(i).GFYeJi);
				mList.add(bxInfo.get(i).GFFaRen);
				mList.add(bxInfo.get(i).GFShiFouChangQiHeZuo);
