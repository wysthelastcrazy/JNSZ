...
public class YingShouHTDetailActivity extends BaseNormalActivity{
	...
	private RspYingShouHTDetailEntity rsp;
...
//	private List<String> mStrList=new ArrayList<String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initExtras();
		initLayout();
		registMsgRecevier(EventCommon.EVENT_YINGSHOU_HT);
...
//		String YWID=entity.YWID;
		int seq=ProtocalManager.getInstance().getYingShouHTDetail(entity);
		mReqList.add(seq);
		...
		if(this.type==Common.TYPE_DAIBAN){
			...
			mBottomView.setClickListener(new IDaiBanClickListener() {
				...
				public void typeClickListener() {
					...
					IntentUtils.starTiJiaoActivity(YingShouHTDetailActivity.this, GET_TYPE,noTag);
				}
				...
				public void nextClickListener() {
					...
					IntentUtils.starJieShouRenActivity(YingShouHTDetailActivity.this, mList, GET_JIESHOUREN);
				}
...
//	};
	...
	public void handleReceiveMsg(int eventId, int seqNo, Object obj) {
		// TODO Auto-generated method stub
		if(eventId==EventCommon.EVENT_YINGSHOU_HT){
			if(obj instanceof RspYingShouHTDetailEntity){
				RspYingShouHTDetailEntity rsp=(RspYingShouHTDetailEntity) obj;
	...
	protected void handleMsg(Message msg) {
		...
		switch (what) {
		case FLAG_SET_TOP:
			 rsp=(RspYingShouHTDetailEntity)msg. obj;
			if(rsp!=null&&rsp.isSucc){
//				List<PShenPiInfoEntity> mList=rsp.mEntity.BXMXInfo;
				ArrayList<PDetailYSHTInfoEntity> bxInfo=rsp.mEntity.yingShouHTPSInfo;
				MyLog.debug(TAG, "[handleMsg] bxInfo:"+bxInfo.size());
...
//			showToast(mBLUserName);
	...
	private View getHeaderView(ArrayList<PDetailYSHTInfoEntity> bxInfo){
		MyLog.debug(TAG, "[getHeaderView] bxInfo:"+bxInfo.size());
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			for(int i=0;i<bxInfo.size();i++){
				MyLog.debug(TAG, "[getHeaderView] index:"+i);
				...
				mList.add(bxInfo.get(i).FaBaoRen);
				mList.add(bxInfo.get(i).ZhuZhuanYe);
				mList.add(bxInfo.get(i).GongChengTouZi+"万元");
...
//				}
			...
			DetailShenPiYiJianTitleView view=new DetailShenPiYiJianTitleView(YingShouHTDetailActivity.this);
			layout.addView(view);
		}
		return layout;
	}
			...
			// TODO Auto-generated method stub
			 ...
			 new Thread() {
					public void run() {
						...
						String url="http://60.208.75.182:9080/file/"+name.replaceAll("\\+", "%20");
						MyLog.debug(TAG, "[IWordOpenListener]  url:"+url+"   filename:"+filename);
