...
import com.DCHZ.TYLINCN.entity.PShenPiInfoEntity;
...
public class TouBiaoDetailActivity extends BaseNormalActivity{
	...
	private RspShenPiInfoEntity rsp;
...
//	private List<String> mStrList=new ArrayList<String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initExtras();
		initLayout();
		registMsgRecevier(EventCommon.EVENT_SHENPI_INFO);
		...
		int seq=ProtocalManager.getInstance().getTouBiaoDetail(entity);
		mReqList.add(seq);
...
//		showToast("type:"+type+"  entity:"+entity.LCID);
		...
		if(this.type==Common.TYPE_DAIBAN){
			...
			mBottomView.setClickListener(new IDaiBanClickListener() {
				...
				public void typeClickListener() {
					...
					IntentUtils.starTiJiaoActivity(TouBiaoDetailActivity.this, GET_TYPE,noTag);
				}
				...
				public void nextClickListener() {
					...
					IntentUtils.starJieShouRenActivity(TouBiaoDetailActivity.this, mList, GET_JIESHOUREN);
				}
	...
	public void handleReceiveMsg(int eventId, int seqNo, Object obj) {
		// TODO Auto-generated method stub
		if(eventId==EventCommon.EVENT_SHENPI_INFO){
			if(obj instanceof RspShenPiInfoEntity){
				RspShenPiInfoEntity rsp=(RspShenPiInfoEntity) obj;
	...
	protected void handleMsg(Message msg) {
		...
		switch (what) {
		case FLAG_SET_TOP:
			 rsp=(RspShenPiInfoEntity)msg. obj;
			if(rsp!=null&&rsp.isSucc){
//				List<PShenPiInfoEntity> mList=rsp.mEntity.BXMXInfo;
				ArrayList<PShenPiInfoEntity> bxInfo=rsp.mEntity.baoZhengJinSQInfo;
				if(bxInfo!=null&&bxInfo.size()>0)
				mHeader.setItem(bxInfo.get(0).SQBuMen);
...
//		MyLog.debug(TAG, "[onActivityResult] resultCode:"+resultCode);
	...
	private View getHeaderView(ArrayList<PShenPiInfoEntity> bxInfo){
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			for(int i=0;i<bxInfo.size();i++){
				...
				mList.add(bxInfo.get(i).SQBZJLeiXing);
				mList.add(bxInfo.get(i).SQDanBaoJinE);
				mList.add(bxInfo.get(i).SQZhiFuFangShi);
