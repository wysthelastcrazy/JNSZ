...
public class JieKuangDetailActivity extends BaseNormalActivity{
	...
	private RspJieKuangDetailEntity rsp;
...
//	private List<String> mStrList=new ArrayList<String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initExtras();
		initLayout();
		registMsgRecevier(EventCommon.EVENT_JIEKUANG_DETAIL);
...
//		String YWID=entity.YWID;
		int seq=ProtocalManager.getInstance().getJieKuangDetail(entity);
		mReqList.add(seq);
		...
		if(this.type==Common.TYPE_DAIBAN){
			...
			mBottomView.setClickListener(new IDaiBanClickListener() {
				...
				public void typeClickListener() {
					...
					IntentUtils.starTiJiaoActivity(JieKuangDetailActivity.this, GET_TYPE,noTag);
				}
				...
				public void nextClickListener() {
					...
					IntentUtils.starJieShouRenActivity(JieKuangDetailActivity.this, mList, GET_JIESHOUREN);
				}
...
//	};
	...
	public void handleReceiveMsg(int eventId, int seqNo, Object obj) {
		// TODO Auto-generated method stub
		if(eventId==EventCommon.EVENT_JIEKUANG_DETAIL){
			if(obj instanceof RspJieKuangDetailEntity){
				RspJieKuangDetailEntity rsp=(RspJieKuangDetailEntity) obj;
	...
	protected void handleMsg(Message msg) {
		...
		switch (what) {
		case FLAG_SET_TOP:
			 rsp=(RspJieKuangDetailEntity)msg. obj;
			if(rsp!=null&&rsp.isSucc){
//				List<PShenPiInfoEntity> mList=rsp.mEntity.BXMXInfo;
				ArrayList<PDetailJKInfoEntity> bxInfo=rsp.mEntity.jkInfo;
				if(bxInfo!=null&&bxInfo.size()>0)
				mHeader.setItem(bxInfo.get(0).JKBuMen);
				mHeader.setItem3(bxInfo.get(0).JKRiQi.split(" ")[0]);
...
//		MyLog.debug(TAG, "[onActivityResult] resultCode:"+resultCode);
	...
	private View getHeaderView(ArrayList<PDetailJKInfoEntity> bxInfo){
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			for(int i=0;i<bxInfo.size();i++){
				...
				mList.add(bxInfo.get(i).JKLeiXing);
				mList.add(bxInfo.get(i).JKYongTu);
//				mList.add(bxInfo.get(i).JKRiQi.split(" ")[0]);
				mList.add(bxInfo.get(i).XMBianHao);
