...
public class XiangMuDetailActivity extends BaseNormalActivity{
	...
	private RspXiangMuDetailEntity rsp;
...
//	private List<String> mStrList=new ArrayList<String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initExtras();
		initLayout();
		registMsgRecevier(EventCommon.EVENT_XIANGMU);
		...
		int seq=ProtocalManager.getInstance().getXiangMuDetail(entity);
		mReqList.add(seq);
		showLoading();
	}
...
//		mMsgPage.setRefreshListener(mRefreshListener);
		if(this.type==Common.TYPE_DAIBAN){
			...
			mBottomView.setClickListener(new IDaiBanClickListener() {
				...
				public void typeClickListener() {
					...
					IntentUtils.starTiJiaoActivity(XiangMuDetailActivity.this, GET_TYPE,noTag);
				}
				...
				public void nextClickListener() {
					...
					if("同意".equals(TJtype)||"退回".equals(TJtype)){
						IntentUtils.starJieShouRenActivity(XiangMuDetailActivity.this, mList, GET_JIESHOUREN);
					}else{
						showToast("请先选择提交方式！");
					}
				}
	...
	public void handleReceiveMsg(int eventId, int seqNo, Object obj) {
		// TODO Auto-generated method stub
		if(eventId==EventCommon.EVENT_XIANGMU){
			if(obj instanceof RspXiangMuDetailEntity){
				RspXiangMuDetailEntity rsp=(RspXiangMuDetailEntity) obj;
	...
	protected void handleMsg(Message msg) {
		...
		switch (what) {
		case FLAG_SET_TOP:
			 rsp=(RspXiangMuDetailEntity)msg. obj;
			if(rsp!=null&&rsp.isSucc){
//				List<PShenPiInfoEntity> mList=rsp.mEntity.BXMXInfo;
				ArrayList<PDetailXiangMuEntity> bxInfo=rsp.mEntity.XiangMuXiaDaInfo;
				if(bxInfo!=null&&bxInfo.size()>0)
//				mHeader.setItem(bxInfo.get(0).BXMXZhuDaoBuMen);
					mHeader.setValue(bxInfo.get(0).ShenDingRen, bxInfo.get(0).XMGuangLiJiBie, bxInfo.get(0).XMBianHao, bxInfo.get(0).XiangMuFuZe);
...
//			showToast(mBLUserID);
	...
	private View getHeaderView(ArrayList<PDetailXiangMuEntity> bxInfo){
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			for(int i=0;i<bxInfo.size();i++){
				...
				mList.add(bxInfo.get(i).JieDuanMingCheng);
				mList.add(bxInfo.get(i).XiaDaRiQi.split(" ")[0]);
