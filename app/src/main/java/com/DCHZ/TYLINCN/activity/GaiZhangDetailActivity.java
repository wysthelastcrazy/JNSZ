...
public class GaiZhangDetailActivity extends BaseNormalActivity{
...
//	private List<String> mStrList=new ArrayList<String>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initExtras();
		initLayout();
		registMsgRecevier(EventCommon.EVENT_GAIZHANG);
...
//		String YWID=entity.YWID;
		int seq=ProtocalManager.getInstance().getGaiZhangDetail(entity);
		mReqList.add(seq);
...
//		mMsgPage.setRefreshListener(mRefreshListener);
		if(this.type==Common.TYPE_DAIBAN){
			...
			mBottomView.setClickListener(new IDaiBanClickListener() {
				...
				public void typeClickListener() {
					...
					IntentUtils.starTiJiaoActivity(GaiZhangDetailActivity.this, GET_TYPE,noTag);
				}
				...
				public void nextClickListener() {
					...
					IntentUtils.starJieShouRenActivity(GaiZhangDetailActivity.this, mList, GET_JIESHOUREN);
				}
	...
	public void handleReceiveMsg(int eventId, int seqNo, Object obj) {
		// TODO Auto-generated method stub
		if(eventId==EventCommon.EVENT_GAIZHANG){
			if(obj instanceof RspGaiZhangEntity){
				RspGaiZhangEntity rsp=(RspGaiZhangEntity) obj;
	...
	protected void handleMsg(Message msg) {
		...
		switch (what) {
		case FLAG_SET_TOP:
			 rsp=(RspGaiZhangEntity)msg. obj;
			if(rsp!=null&&rsp.isSucc){
//				List<PShenPiInfoEntity> mList=rsp.mEntity.BXMXInfo;
				ArrayList<PDetailGaiZhangEntity> bxInfo=rsp.mEntity.GaiZhangShenQingInfo;
				if(bxInfo!=null&&bxInfo.size()>0)
				mHeader.setValue(bxInfo.get(0).GZShenQingRen, bxInfo.get(0).GZShenQingBuMen, bxInfo.get(0).XMBianHao, bxInfo.get(0).GZLeiXing);
...
//			showToast(mBLUserName);
	...
	private View getHeaderView(ArrayList<PDetailGaiZhangEntity> bxInfo){
		...
		if(bxInfo!=null&&bxInfo.size()>0){
			for(int i=0;i<bxInfo.size();i++){
				...
				mList.add(bxInfo.get(i).GZYongTu);
				mList.add(bxInfo.get(i).GZQianYueDanWei);
				mList.add(bxInfo.get(i).GZWenJianLeiBie);
				mList.add(bxInfo.get(i).GZHeTongBianHao);
				mList.add(bxInfo.get(i).GZFenShu);
				mList.add(bxInfo.get(i).GZShiFouGuiDang);
...
//				}
			...
			DetailShenPiYiJianTitleView view=new DetailShenPiYiJianTitleView(GaiZhangDetailActivity.this);
			layout.addView(view);
		}
		return layout;
	}
